...
only a balance (no transactions) cannot be audited this way; rather than silently
skip it, the report surfaces it as an explicit "can't audit" note.
    ...
    for debt in debt_accounts:
        ...
        for txn in transactions:
            if txn.get("account_id") != debt.account_id:
                continue
            if txn.get("pending"):
                # A pending posting has not actually reached the loan yet, so it
                # cannot satisfy (or fail) a cleared-payment audit. Skip it for
            ...
            if d is None or cents is None:
                # A debt-account row we cannot parse is not silently ignored: it is
            ...
            flags.append(
                {
                    ...
                    "shortfall": None,
                    "detail": (
                        "This debt syncs a balance but no transactions, so its "
                        "payments can't be verified here."
                    ),
                }
            )
            continue
